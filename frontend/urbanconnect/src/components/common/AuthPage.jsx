...
export default function AuthPage() {
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    if (validateForm()) {
      console.log("Validation passed", formData, userType);
      setIsLoading(true);
      try {
        console.log(
          "Sending signup request to http://localhost:5000/profile/users/signup"
        );
        const response = await fetch(
          "http://localhost:5000/profile/users/signup",
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          toast.success(
            result.message || "User created successfully! Please log in.",
            {
              position: "top-center",
              autoClose: 1000,
  ...
  const handleSignInSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await fetch(
          "http://localhost:5000/profile/users/login",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
